...
class WXBizMsgCrypt
{
	...
	public function __construct($token, $encodingAesKey, $appId)
	{
		...
		$this->appId = $appId;
		$this->pc=new PrpCrypt($this->encodingAesKey);
	}
	...
	{
		//加密
		$array = $this->pc->encrypt($replyMsg, $this->appId);
		$ret = $array[0];
		if ($ret != 0) {
			return $ret;
		}
	...
	public function decryptMsg($msgSignature, $timestamp=null, $nonce, $postData, &$msg)
	{
		...
		$result = $this->pc->decrypt($encrypt, $this->appId);
		if ($result[0] != 0) {
			return $result[0];
		}
		$msg = $result[1];
	 ...
	 * @return int 成功0，失败返回对应的错误码
	 */
	public function ticketDecryptMsg($msgSignature, $timestamp=null, $nonce, $postData, &$msg)
	{
		if (strlen($this->encodingAesKey) != 43) {
			return ErrorCode::$IllegalAesKey;
		}
		//提取密文
		$xmlparse = new XMLParse;
		$array = $xmlparse->componentVerifyTicket($postData);
		$ret = $array[0];
		if ($ret != 0) {
			return $ret;
		}
